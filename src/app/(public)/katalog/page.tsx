/* Hallmark · page: Katalog · genre: modern-minimal · macrostructure: Workbench · design-system: design.md */
          ...
          <table className="w-full text-left text-sm border-collapse border border-slate-200">
            ...
            <tbody>
              {products.slice(0, 40).map((p: any) => {
                ...
                const availableStock = initialVariant ? Math.max(0, initialVariant.stock - initialVariant.reservedStock) : 0;
                const stockStatus = p.isReadyStock && availableStock > 0 ? "Ready Stock" : (p.isReadyStock ? "Habis" : "Pre-order");
                ...
                return (
                  <tr key={p.id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 border border-[var(--color-rule)]/50 font-medium text-blue-600">
                      <a href={`/katalog/${p.slug}`} className="hover:underline focus-visible:outline-2 focus-visible:outline-[var(--color-focus)] rounded-sm">{p.name}</a>
                    </td>
