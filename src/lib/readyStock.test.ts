...
describe("readyStock — getReadyStockSummary", () => {
  ...
  it("ready stock tapi isRequestPricing: true → state rfq", () => {
    const s = getReadyStockSummary({
      isReadyStock: true,
      isRequestPricing: true,
      variants: [v({ id: "a", price: 1_500_000, stock: 5 })],
    });
    expect(s.state).toBe("rfq");
  });

  it("ready stock tapi semua varian harganya 0 (belum diset) → state rfq, bukan sold_out", () => {
    const s = getReadyStockSummary({
      isReadyStock: true,
      variants: [v({ id: "a", price: 0, stock: 5 })],
    });
    expect(s.state).toBe("rfq");
  });
});
